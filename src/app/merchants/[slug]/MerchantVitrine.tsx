...
export default function MerchantVitrine({ merchantSlug }: MerchantVitrineProps) {
  ...
  const produtosMerchant = produtos.filter(p => p.merchantId === merchant?.id);
  const categoriasMerchant = categorias.filter(c => c.merchantId === merchant?.id);
  ...
  const adicionarAoCarrinho = (produto: typeof produtosMerchant[0]) => {
    adicionarItem({
      tipo: 'produto',
